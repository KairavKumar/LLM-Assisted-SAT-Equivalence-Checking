# 16-bit addmult-style design notes

All variants compute `(a + b + c)^3` for 16-bit unsigned inputs and produce a 53-bit output.
...
- Uses mixed multipliers: 16x16 product lanes alternate array and tree styles.
...
## matrix_lane_cube.v
- Matrix-lane decomposition: form nine 16x16 lanes `aa ab ac / ba bb bc / ca cb cc`, reduce row-wise to `r0 r1 r2`, reduce to `sq`, form `s = a + b + c`, then `y = sq*s`.
