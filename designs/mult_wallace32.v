// 32-bit Wallace tree multiplier (2-input) using CSA reduction.
module mult_wallace32 (
    ...
    wire [63:0] c1 [0:10];
    generate
        for (i = 0; i < 11; i = i + 1) begin : gen_s1
            csa64 u_csa1 (
                .a(pp[i*3]),
                .b(pp[i*3 + 1]),
                .c(pp[i*3 + 2]),
    ...
    wire [63:0] op2 [0:23];
    generate
        for (i = 0; i < 11; i = i + 1) begin : gen_op2
            assign op2[i] = s1[i];
            assign op2[i+11] = {c1[i][62:0], 1'b0};
        end
    endgenerate
    assign op2[22] = pp[30];
    assign op2[23] = pp[31];
    ...
    wire [63:0] op3 [0:15];
    generate
        for (i = 0; i < 8; i = i + 1) begin : gen_op3
            assign op3[i] = s2[i];
            assign op3[i+8] = {c2[i][62:0], 1'b0};
        end
    endgenerate
    ...
    wire [63:0] c6 [0:1];
    generate
        for (i = 0; i < 2; i = i + 1) begin : gen_s6
            csa64 u_csa6 (
                .a(op6[i*3]),
                .b(op6[i*3 + 1]),
                .c(op6[i*3 + 2]),
    ...
    wire [63:0] op7 [0:3];
    generate
        for (i = 0; i < 2; i = i + 1) begin : gen_op7
            assign op7[i] = s6[i];
            assign op7[i+2] = {c6[i][62:0], 1'b0};
        end
    endgenerate
